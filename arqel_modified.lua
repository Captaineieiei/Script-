...
-- theme
Arqel.Theme = {
    Accent = Color3.fromRGB(115, 75, 170),
    AccentHover = Color3.fromRGB(145, 105, 200),
    Background = Color3.fromRGB(11, 9, 18),
    Header = Color3.fromRGB(18, 14, 28),
    Input = Color3.fromRGB(24, 19, 38),
    Text = Color3.fromRGB(240, 235, 255),
    TextDim = Color3.fromRGB(130, 115, 158),
    Success = Color3.fromRGB(50, 205, 110),
    Error = Color3.fromRGB(245, 70, 90),
    Warning = Color3.fromRGB(255, 180, 50),
    StatusIdle = Color3.fromRGB(110, 85, 148),
    Discord = Color3.fromRGB(88, 101, 242),
    DiscordHover = Color3.fromRGB(114, 137, 218),
    Divider = Color3.fromRGB(55, 42, 82),
    Pending = Color3.fromRGB(50, 40, 68)
}
...
local function CreateUserInfoPanel(parent, windowWidth, panelHeight, panelWidth, mainFrame, gap, startOpen)
    ...
    keyContainer.Size = UDim2.new(1, 0, 0, fieldHeight)
    keyContainer.BackgroundTransparency = 1
    keyContainer.LayoutOrder = 9
    keyContainer.Parent = contentFrame

    local keyTitle = Instance.new("TextLabel")
    keyTitle.Size = UDim2.new(1, 0, 0, 11)
    keyTitle.BackgroundTransparency = 1
    keyTitle.Text = "Key"
    keyTitle.TextColor3 = Arqel.Theme.TextDim
    keyTitle.TextSize = titleSize
    keyTitle.Font = Enum.Font.ArimoBold
    keyTitle.TextXAlignment = Enum.TextXAlignment.Left
    keyTitle.Parent = keyContainer

    local keyValue = Instance.new("TextLabel")
    keyValue.Size = UDim2.new(1, -(copyBtnSize + 6), 0, 14)
    keyValue.Position = UDim2.new(0, 0, 0, 11)
    keyValue.BackgroundTransparency = 1
    keyValue.Text = "—"
    keyValue.TextColor3 = Arqel.Theme.Accent
    keyValue.TextSize = isCompact and 9 or 10
    keyValue.Font = Enum.Font.ArimoBold
    keyValue.TextXAlignment = Enum.TextXAlignment.Left
    keyValue.TextTruncate = Enum.TextTruncate.AtEnd
    keyValue.Parent = keyContainer

    local keyCopyBtn = Instance.new("ImageButton")
    keyCopyBtn.Size = UDim2.new(0, copyBtnSize, 0, copyBtnSize)
    keyCopyBtn.Position = UDim2.new(1, 0, 0.5, 1)
    keyCopyBtn.AnchorPoint = Vector2.new(1, 0.5)
    keyCopyBtn.BackgroundTransparency = 1
    keyCopyBtn.Image = getIcon("copy")
    keyCopyBtn.ImageColor3 = Arqel.Theme.TextDim
    keyCopyBtn.ScaleType = Enum.ScaleType.Fit
    keyCopyBtn.Parent = keyContainer
    keyCopyBtn.MouseEnter:Connect(function() TweenService:Create(keyCopyBtn, TweenInfo.new(0.15), {ImageColor3 = Arqel.Theme.Accent}):Play() end)
    keyCopyBtn.MouseLeave:Connect(function() TweenService:Create(keyCopyBtn, TweenInfo.new(0.15), {ImageColor3 = Arqel.Theme.TextDim}):Play() end)
    keyCopyBtn.MouseButton1Click:Connect(function()
        local k = getgenv().SCRIPT_KEY or ""
        if k ~= "" then
            pcall(function() setclipboard(k) end)
            TweenService:Create(keyCopyBtn, TweenInfo.new(0.1), {ImageColor3 = Arqel.Theme.Success}):Play()
            task.delay(0.3, function() TweenService:Create(keyCopyBtn, TweenInfo.new(0.15), {ImageColor3 = Arqel.Theme.TextDim}):Play() end)
            Arqel:Notify("Copied", "Key copied to clipboard", 2, "copy")
        end
    end)

    local divider4 = Instance.new("Frame")
    divider4.Size = UDim2.new(1, 16, 0, 2)
    divider4.Position = UDim2.new(0.5, 0, 0, 0)
    divider4.AnchorPoint = Vector2.new(0.5, 0)
    divider4.BackgroundColor3 = Arqel.Theme.Divider
    divider4.BorderSizePixel = 0
    divider4.LayoutOrder = 10
    divider4.Parent = contentFrame

    -- Expiry field
    local expiryContainer = Instance.new("Frame")
    expiryContainer.Size = UDim2.new(1, 0, 0, fieldHeight)
    expiryContainer.BackgroundTransparency = 1
    expiryContainer.LayoutOrder = 11
    expiryContainer.Parent = contentFrame

    local expiryTitle = Instance.new("TextLabel")
    expiryTitle.Size = UDim2.new(1, 0, 0, 11)
    expiryTitle.BackgroundTransparency = 1
    expiryTitle.Text = "Expires"
    expiryTitle.TextColor3 = Arqel.Theme.TextDim
    expiryTitle.TextSize = titleSize
    expiryTitle.Font = Enum.Font.ArimoBold
    expiryTitle.TextXAlignment = Enum.TextXAlignment.Left
    expiryTitle.Parent = expiryContainer

    local expiryValue = Instance.new("TextLabel")
    expiryValue.Size = UDim2.new(1, 0, 0, 14)
    expiryValue.Position = UDim2.new(0, 0, 0, 11)
    expiryValue.BackgroundTransparency = 1
    expiryValue.Text = "—"
    expiryValue.TextColor3 = Arqel.Theme.Warning
    expiryValue.TextSize = isCompact and 9 or 10
    expiryValue.Font = Enum.Font.ArimoBold
    expiryValue.TextXAlignment = Enum.TextXAlignment.Left
    expiryValue.TextTruncate = Enum.TextTruncate.AtEnd
    expiryValue.Parent = expiryContainer

    local divider5 = Instance.new("Frame")
    divider5.Size = UDim2.new(1, 16, 0, 2)
    divider5.Position = UDim2.new(0.5, 0, 0, 0)
    divider5.AnchorPoint = Vector2.new(0.5, 0)
    divider5.BackgroundColor3 = Arqel.Theme.Divider
    divider5.BorderSizePixel = 0
    divider5.LayoutOrder = 12
    divider5.Parent = contentFrame
    ...
    local function formatExpiry(raw)
        ...
        if ts then
            ...
            if d > 0 then return d .. "d " .. h .. "h left"
            elseif h > 0 then return h .. "h " .. m .. "m left"
            else return m .. "m left" end
        end
        return tostring(raw)
    end
    ...
    task.spawn(function()
        while clockRunning do
            ...
            clockTimeLabel.Text = formatTime12()
            clockDateLabel.Text = formatDate()
            -- update key and expiry
            local k = getgenv().SCRIPT_KEY or ""
            if k ~= "" and k ~= "KEYLESS" then
                keyValue.Text = k:sub(1, 6) .. string.rep("•", math.max(0, #k - 6))
            else
                keyValue.Text = "—"
            end
            local expRaw = getgenv().SCRIPT_KEY_EXPIRY
            expiryValue.Text = formatExpiry(expRaw)
            task.wait(1)
        end
    end)
...
local function BuildKeyUI()
    ...
    local function handleRedeem()
        ...
        if valid then
            ...
            if type(result) == "table" then
                local exp = result.expiry or result.expires or result.expires_at or result.expiration or nil
                if exp then getgenv().SCRIPT_KEY_EXPIRY = tostring(exp) else getgenv().SCRIPT_KEY_EXPIRY = nil end
            end
