...
local function SetAllVisible(obj,v) for _,k in ipairs(_drawKeys) do pcall(function() obj[k].Visible=v end) end end
...
RunService.RenderStepped:Connect(function()
    ...
    local toRemove, frameThickness, frameBoxColor, frameVpY = {}, CFG.BoxThickness, CFG.BoxColor, Camera.ViewportSize.Y

    for c2,obj in pairs(ESPObjects) do
        if not c2 or not c2.Parent then table.insert(toRemove,c2); continue end
        ...
        local dist = myHRP and (myHRP.Position-hrp2.Position).Magnitude or math.huge
    ...
    for _,c2 in ipairs(toRemove) do RemoveESP(c2) end
end)

local function cleanPlayerESP(plr) for oldChar,obj in pairs(ESPObjects) do if obj.plr==plr then RemoveESP(oldChar) end end end
local function spawnESP(c2,plr)
    task.spawn(function()
        c2:WaitForChild("HumanoidRootPart",10); c2:WaitForChild("Humanoid",10)
...
task.spawn(function()
    ...
    LP.CharacterAdded:Connect(function()
        ...
        for _,plr in pairs(Players:GetPlayers()) do
            if plr~=LP and plr.Character and not ESPObjects[plr.Character] then spawnESP(plr.Character,plr) end
        end
        ScanWorkspace()
    end)
end)
...
local function loadBackpackItems()
    ...
    if ok and scrolling then
        for _,slot in ipairs(scrolling:GetChildren()) do
            ...
            if not slot:IsA("UIBase") and not slot:IsA("UIComponent") and name~="" and name~="Animation" then
                local max=60; local nl=name:lower()
                if nl:find("stone") or nl:find("gold") or nl:find("iron") then max=500 end
                pcall(function()
                    local a=slot:FindFirstChild("Amout")
                ...
                pcall(function()
                    ...
                    if img and img:IsA("ImageLabel") and img.Image~="" then icon=img.Image end
                end)
                table.insert(titles, name); itemMap[name]={Title=name, value=name, max=max, icon=icon}
            end
        end
    end
...
local itemIconAssets = {}

local function patchDropdownIcons()
    task.spawn(function()
        task.wait(0.25)
        local dropFrame=nil
        pcall(function() dropFrame=ItemDropdown.Frame or ItemDropdown.Instance or ItemDropdown.Object or ItemDropdown.Content end)
        if not dropFrame then return end
        for _,v in ipairs(dropFrame:GetDescendants()) do
            if v:IsA("ImageLabel") and itemIconAssets[v.Image] then v.Size=UDim2.fromOffset(32,32) end
        end
    end)
end
...
local function createItemDropdown(titles, itemMap)
    ...
    for _,name in ipairs(titles) do
        ...
        if entry and entry.icon then itemIconAssets[entry.icon]=true; table.insert(values,{Title=name,Icon=entry.icon})
        else table.insert(values, name) end
    end
...
local function getSlotCount(slot)
    ...
    if not lbl or not lbl.Parent then
        lbl=slot:FindFirstChild("Amout")
        if not lbl then for _,child in ipairs(slot:GetChildren()) do if child:IsA("TextLabel") or child:IsA("TextButton") then lbl=child; break end end end
...
FarmSection:Toggle({
    ...
    Callback=function(state)
        ...
        task.spawn(function()
            ...
            if not DepositRemote then FarmRunning=false; return end
            local farmScrolling
...
ESPMain:Toggle({
    ...
    Callback=function(state)
        ...
        if not state then
            ...
            for c2 in pairs(ESPObjects) do if not c2 or not c2.Parent then RemoveESP(c2) end end
...
local Locations = {
    {Title="🛒 ที่ขายของ",        Position=Vector3.new(10559,-328,-3839)},
    {Title="🌶️ เก็บพริก",        Position=Vector3.new(7319,-295,-616)},
    {Title="🥕 เก็บแครอท",        Position=Vector3.new(10311,-293,27)},
    {Title="🥛 เก็บนม",           Position=Vector3.new(-3782,-295,3153)},
    {Title="🍅 เก็บมะเขือเทศ",   Position=Vector3.new(-9181,-260,-238)},
    ...
    {Title="การาจกลาง",           Position=Vector3.new(260,-320,-1888)},
}
...
local function setFullbright(state)
    ...
    if state then
        ...
        _fbConn = RunService.RenderStepped:Connect(function()
            ...
            if Lighting.Ambient~=Color3.new(1,1,1) or Lighting.GlobalShadows then applyFullbright() end
        end)
    else
