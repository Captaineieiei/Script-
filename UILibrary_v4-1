--[[
    Pro Mobile & PC UI Library v4 (Visual Refresh)
    ================================================
    API เหมือน v3 ทุกอย่าง (:Set/:Get/:SetVisible/:Destroy, Flag, Library:Notify,
    Tab:CreateSection, ToggleKeybind, Window:Destroy/SetTitle/Toggle)
    เปลี่ยนแค่ "หน้าตา" ให้สวยขึ้น:
      - Accent เป็น gradient ฟ้า->ม่วง (ใช้กับ slider / toggle / แท็บ active)
      - แท็บ sidebar ดีไซน์ใหม่ มีแถบเรืองซ้ายตอน active + รองรับ Icon (emoji)
      - การ์ด element มีขอบเรืองบางๆตอน hover ให้มีมิติ
      - มุมโค้ง 12px ทั่วทั้ง UI
      - Notify มี 4 ชนิดสี (success/error/warning/info) + แถบสีข้าง + ไอคอน
      - จุดโลโก้เล็กๆ หน้า Title
]]
...
local Theme = {
    ...
    AccentA = Color3.fromRGB(90, 130, 255),   -- gradient start (blue)
    AccentB = Color3.fromRGB(170, 100, 255),  -- gradient end (purple)
...
Theme.Accent = Theme.AccentA -- ค่าเดี่ยวสำรอง เผื่อจุดที่ต้องใช้สีเดียว
...
-- ไล่เฉดสี accent มุม 100 องศา ใส่กับ inst ไหนก็ได้
...
-- glow-stroke เมื่อ hover การ์ด element (ความรู้สึกมีมิติ)
local function applyGlowOnHover(frame)
    local glow = stroke(frame, Theme.AccentA, 1)
    glow.Transparency = 1
...
function Library:CreateWindow(config)
    ...
    corner(TopBar, 12)
    -- ปิดมุมโค้งด้านล่างของ TopBar (ให้เหลี่ยมชนกับด้านล่าง)
    ...
    TabList.ScrollBarImageColor3 = Theme.AccentA
    TabList.Active = true
    ...
    function Window:CreateTab(name, icon)
        ...
        ActiveBar.BackgroundColor3 = Theme.AccentA
        ...
        TabContent.ScrollBarImageColor3 = Theme.AccentA
        ...
        TabBtn.MouseButton1Click:Connect(function()
            if CurrentTab then
                ...
                if CurrentTab.SetActive then CurrentTab.SetActive(false) end
            end
            closeActivePopup()
            setActive(true)
        ...
        function Tab:CreateColorPicker(c)
            ...
            stroke(colorPreview, Theme.Stroke)
