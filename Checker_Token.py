...
def print_banner_v4():
    clr()
    # ── ชื่อโปรแกรมแบบ block สั้นพอดีจอมือถือ ≤40 ตัว ──
    lines = [
        (" ██████╗██╗  ██╗███████╗ ██████╗██╗  ██╗",  '\033[91m'),
        ("██╔════╝██║  ██║██╔════╝██╔════╝██║ ██╔╝",  '\033[93m'),
        ("██║     ███████║█████╗  ██║     █████╔╝ ",  '\033[92m'),
    ...
    print()
    for text, color in tok:
        print(f" {color}{text}{C.RESET}")
    print()
    print(f" {C.BG_RED}{C.WHITE}{C.BOLD} ══ V4.0  HARDCORE EDITION ══ {C.RESET}")
    print(f" {C.DIM}{'─'*40}{C.RESET}")
    print(f" {C.YELLOW}⚡{C.RESET} {C.CYAN}Multi-Thread{C.RESET} {C.DIM}│{C.RESET} {C.MAGENTA}Deep Scan{C.RESET} {C.DIM}│{C.RESET} {C.GREEN}Mass Scan{C.RESET} {C.YELLOW}⚡{C.RESET}")
    print(f" {C.DIM}{'─'*40}{C.RESET}\n")
